    ...
    @forelse ($collection as $item)
        <div class="p-2 m-2 border-b-2 border-[#ff7900] flex justify-between items-center" >
            ...
            <div class="flex flex-row space-x-4">
                <a href="{{route('admin.newton.edit', ['id' => $item->id])}}" class="p-1 font-semibold text-blue-500 border-2 border-blue-500 rounded-lg justify-end hover:text-white hover:bg-blue-500  "><button>Úprava</button></a>

            <form action="{{route('admin.newton.delete', ['id' => $item->id])}}" method="POST">
                @csrf
                @method('delete')
